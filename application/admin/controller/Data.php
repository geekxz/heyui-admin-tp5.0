...
class Data extends Base
{
    ...
    public function index()
    {
        ...
        $Tables_in_geekxz = 'Tables_in_'.config('database.database');
        // var_dump($Tables_in_geekxz);die;
        // echo "<pre>";
        foreach($tables as $key=>$vo){			
            $sql = "select count(0) as alls from ".'`'.$vo[$Tables_in_geekxz].'`';
            $tables[$key]['alls'] = db()->query($sql)['0']['alls'];
            // var_dump($tables);die;
            $operate = [
                '备份' => "javascript:importData('".$vo[$Tables_in_geekxz]."', ".$tables[$key]['alls'].")",
                '还原' => "javascript:backData('".$vo[$Tables_in_geekxz]."')"
            ];
            $tables[$key]['operate'] = showOperate($operate);
            if(file_exists(config('back_path') . $vo[$Tables_in_geekxz] . ".sql")){
                $tables[$key]['ctime'] = date('Y-m-d H:i:s', filemtime(config('back_path') . $vo[$Tables_in_geekxz] . ".sql"));
            }else{
                $tables[$key]['ctime'] = '无';
            }
        }
